 ...
 * Version ADMIN - Utilise l'accès direct sans authentification
 *
 * Usage:
 * node scripts/addAcademicYearToStudentsAdmin.mjs --dry-run  (lecture seule)
 * node scripts/addAcademicYearToStudentsAdmin.mjs --write    (écriture réelle)
 */
...
async function migrateStudents() {
  ...
  const universityId = process.env.UNIVERSITY_ID || 'univ-universite-nice-sophia-antipol-2026';
  ...
  try {
    // Récupérer tous les étudiants directement
    ...
    if (!studentsSnap.exists()) {
      ...
      console.log(`   Chemin: universities/${universityId}/students`);
      return;
    }
    ...
    studentsToMigrate.slice(0, 5).forEach(([id, student]) => {
      console.log(`   - ${student.firstName} ${student.lastName} (${student.level}) - ${student.matricule || 'N/A'}`);
    });
    ...
    if (isDryRun) {
      ...
      console.log('   node scripts/addAcademicYearToStudentsAdmin.mjs --write\n');
      return;
    }
    ...
    for (const [studentId, student] of studentsToMigrate) {
      try {
        ...
        successCount++;
        console.log(`   ✅ ${student.firstName} ${student.lastName} (${student.matricule || 'N/A'})`);
      } catch (error) {
        errorCount++;
