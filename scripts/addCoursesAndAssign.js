/**
 * Script pour ajouter 10 cours, assigner 5 à un enseignant et inscrire des étudiants
 * Usage: node scripts/addCoursesAndAssign.js
 */
...
async function findTeacher(email) {
  ...
  try {
    ...
    console.log(`✅ Enseignant trouvé: ${userRecord.displayName} (${userRecord.uid})`);
    return userRecord;
  } catch (error) {
    console.error(`❌ Enseignant non trouvé: ${error.message}`);
    return null;
  }
}

async function findUniversityId(teacherUid) {
  console.log(`🔍 Recherche de l'université de l'enseignant...`);

  const userRef = db.ref(`users/${teacherUid}`);
  const snapshot = await userRef.once('value');

  if (snapshot.exists()) {
    const universityId = snapshot.val().universityId;
    console.log(`✅ Université trouvée: ${universityId}`);
    return universityId;
  }

  console.error(`❌ Université non trouvée pour l'enseignant`);
  return null;
}
...
async function findStudents(universityId, count = 2) {
  ...
  if (snapshot.exists()) {
    ...
    students.forEach(s => console.log(`   - ${s.firstName} ${s.lastName} (${s.studentNumber})`));
...
async function main() {
  try {
    ...
    const teacher = await findTeacher('enseignant.test@nice.fr');
    if (!teacher) {
      console.error('❌ Impossible de continuer sans enseignant');
      process.exit(1);
    }

    // 2. Trouver l'université
    const universityId = await findUniversityId(teacher.uid);
    if (!universityId) {
      console.error('❌ Impossible de continuer sans université');
      process.exit(1);
    }
    ...
    console.log(`   • ${students.length} étudiant(s) inscrit(s) à chaque cours`);
